...
var USE_GATE = false;            /* ⭐⭐ 0818 밤 소로 — 게이트를 내렸다.
                                    ⚠ 이 장치는 「창 열 때 파리가 비친다」를 막으려고 지었는데,
                                      그 증상의 진짜 원인은 타일이 아니라 **궤도**였다(enter 주석 참조).
                                      원인을 고치고 나니 막을 것이 없어졌다 — 병은 나았는데 붕대만 남았다.
                                    ⚠ 게다가 실물과 반대였다. 실제 여객기는 이착륙 때 덮개를 **연다**.
                                      내가 그 사실을 주석에 적어 두고서 닫는 장치를 지었다.
                                    ⭐ 아래 게이트 몸통은 걷지 않고 남긴다 — 느린 연결에서 창이
                                      밋밋한 밑그림만 비추는 일이 잦으면 이 값 하나만 true 로 올린다.
                                    ⚠ 손님이 직접 여닫는 창 덮개(24호)는 그대로다. 이건 자동 게이트만 끈 것이다. */
...
/* ── 집주인 쪽 겉옷 ────────────────────────────────────────────
   ⚠ terra 의 UI 는 서른 몇 개다 (타륜 · 핀 · 라벨 · 시계 · 카드 · 나침반 …).
     하나씩 배선하면 반드시 하나를 빠뜨리고, 새 물건이 생길 때마다 또 와야 한다
...
function mountHtml(){
  ...
  /* ⚠ 나가는 문은 방 밖에 둔다 — 방 안에 두면 #vesperRoom:not(.cabin) 규칙에 걸려
     창을 덮은 동안 함께 사라진다. 들어온 손님은 언제든 나갈 수 있어야 한다. */
...
function enter(hostViewer, spot){
  ...
  /* ⚠⚠ terra 의 enterRoomUrl 은 방에 들 때 useDefaultRenderLoop = false 로 지구를 재운다.
     액자 판에서는 옳았다 — 액자가 제 지구를 따로 띄웠으니 뒤의 지구는 잠들어야 했다.
