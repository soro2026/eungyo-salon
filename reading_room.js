...
(function () {
  ...
  var ROUTES = [{
    ...
    craft: "bre",
    name: "파리 저공 관광",
    face: "센강과 언덕을 낮게 도는 8자 · 브레게 14",
    kind: "tour",
    /* ⚠⚠ 파리는 평평하다 — 지면이 해발 30~130m 안에서만 논다. 그래서 felt(체감)로
       속도를 조절할 여지가 거의 없고, 실질 속도는 아래 minKmh 가 정한다.
       felt 340 × rel 0.24km = 82km/h. 몽마르트르 언덕에서 rel 이 줄면 바닥 80 이 이긴다.
       ⭐ 25호(체감 일정)를 안 어긴다 — 평평한 곳에서 체감이 일정하면 속도도 일정하다. */
    felt: 340,
    /* ⭐ 0822d 신설 — 속도 바닥을 **노선이 정한다.** 안 적으면 지금까지의 120 이다.
       ⚠ 브레게 14 의 실속은 82km/h(Aeropedia). 80 은 그 아래다 —
         0821 소로 판정(「사실 고증 포기하고, 생텍쥐페리와 어디를 날아도 만족한다」)을 여기 적용했다.
    ...
    legs: [
      ...
      /* ⚠ 마지막 선회 — 여기서 북으로 올라 콩코르드로 간다. ⭐ 이 구간이 앵발리드~오르세 를
         가로질러 8자가 닫힌다. 없으면 몽파르나스에서 111° 로 되꺾인다 */
    ...
    var off = viewer.clock.onTick.addEventListener(function () {
      try {
        ...
        /* ⭐ 0822d — 바닥을 **노선이 정한다.** 안 적으면 지금까지의 120 이다(알프스·안데스 그대로).
           ⚠ 파리는 80 이다. 브레게 14 의 실속(82km/h)보다 낮지만, 0821 소로 판정
             「사실 고증 포기」를 적용한 값이다. 저공 관광의 손맛이 실속표보다 앞선다. */
        var kmh = Math.max(route.minKmh || 120, Math.min(route.felt * (spdH / 1000), 900));
